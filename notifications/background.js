...
function getUpComingMeeting(current) {
  chrome.tabs.executeScript(current.id, {
    ...
    if (result != "" && JSON.parse(localStorage.min5) && result[0].includes(" 5 min") && !stop5) {
      new Notification(`Reminder`, {
        icon: '48.png',
        body: `${result[0]}`
      });
    ...
    if (result != "" && JSON.parse(localStorage.min10) && result[0].includes(" 10 min") && !stop10) {
      new Notification(`Reminder`, {
        icon: '48.png',
        body: `${result[0]}`
      });
    ...
    if (result != "" && JSON.parse(localStorage.min15) && result[0].includes("15 min") && !stop15) {
      new Notification(`Reminder`, {
        icon: '48.png',
        body: `${result[0]}`
      });
    ...
    if (result != "" && JSON.parse(localStorage.iscust) && result[0].includes(" " + localStorage.custmin + " min") && !stopcust) {
      new Notification(`Reminder`, {
        icon: '48.png',
        body: `${result[0]}`
      });
